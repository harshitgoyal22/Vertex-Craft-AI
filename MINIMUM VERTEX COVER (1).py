...
nodes=0
edge_prob = 0.00
layout = [    [sg.Text('Enter the number of nodes:', font=('Helvetica', 16),text_color='black')],
    [sg.Input(key='nodes', font=('Helvetica', 16),text_color='black')],
    [sg.Text('Enter the edge probability:', font=('Helvetica', 16),text_color='black')],
    [sg.Input(key='edge_prob', font=('Helvetica', 16),text_color='black')],
    [sg.Button("OK", font=('Helvetica', 16), pad=(20, 20),button_color=('#0B2447', '#EBD8B2'))]
]
window = sg.Window('MINIMUM VERTEX COVER',  layout,size=(500,300), background_color='#b1f2ff')
while True:
...
def LS1_SA(G, cutoff,randSeed):
    ...
    while((time.time() - start_time) < cutoff):
        # currnum.append(len(S))
        # currtime.append(time.time()-start_time)
        T=0.95 * T 
...
currnum=[]
currtime=[]
sol, trace = LS1_SA(G,5,20)
SimulatedAnnealing_soln=len(sol)
print(len(sol))
sol
pltp.plot(currtime,currnum)
pltp.ylabel('Number of nodes')
# naming the y axis
pltp.xlabel('time taken')
  
# giving a title to my graph
pltp.title('Simulated annealing')
...
currnum=[]
currtime=[]
soln2= Hill(G2,nodes,edges_cnt,0.002,5)
HillClimbing_soln=len(soln2)
print(len(soln2))
# soln2
...
#         return
    ...
    for j in range(len(soln)):
        if(soln[j] == 1):
            result.append(j)
    currnum.append(len(result))
    currtime.append(time.time()-startttime)
...
layout=[[sg.Text('Simulated Anealing: ',font=('Arial',15),text_color='black'),sg.Text(str(SimulatedAnnealing_soln),font=('Arial',15),text_color='black')],
        [sg.Text('Genetic Algorithm: ',font=('Arial',15),text_color='black'),sg.Text(str(GA_soln),font=('Arial',15),text_color='black')],
        [sg.Text('Hill Climbing: ',font=('Arial',15),text_color='black'),sg.Text(str(HillClimbing_soln),font=('Arial',15),text_color='black')],
        [sg.Text('Branch and Bound: ',font=('Arial',15),text_color='black'),sg.Text(str(BnB_soln),font=('Arial',15),text_color='black')],
        # [sg.Button('Quit',size=(4,1.2))]
        ]


window = sg.Window('MINIMUM VERTEX COVER',  layout,size=(500,300), background_color='#b1f2ff',element_justification='c')
while True:
    event, values = window.read()
    if event == sg.WINDOW_CLOSED:
        break
